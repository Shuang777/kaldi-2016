...
if [ $# != 3 ]; then
  echo "Usage: swbd1_data_prep.sh /path/to/SWBD data/ swbd"
  exit 1; 
fi 
...
dir=$datadir/local/$name
mkdir -p $dir


# Audio data directory check
if [ ! -d $SWBD_DIR ]; then
  echo "Error: $0 requires a directory argument"
  exit 1; 
fi  
...
find $SWBD_DIR -iname "$fileext" | sort > $dir/sph.flist
...
awk '{for (i=2; i<=NF; i++) count[$i]+=1; } END{for (i in count) print i,count[i]}' \
  $dir/text | sort -k2 -n -r > $dir/word.count
...
sed -e 's?.*/??' -e $regexp $dir/sph.flist | paste - $dir/sph.flist \
  > $dir/sph.scp
...
if [ $fileext == '*.sph' ]; then
  ...
  awk '{
    printf("%s-A sox -t wav %s -t wav - remix 1 |\n", $1, $2); 
    printf("%s-B sox -t wav %s -t wav - remix 2 |\n", $1, $2);
  }' < $dir/sph.scp | sort > $dir/wav.scp || exit 1;
...
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel; do
  cp $dir/$f $datadir/$name/$f || exit 1;
done
