...
if [ -f $data/segments ]; then
  ...
  if [ ! -z $passphrase ]; then
    ...
    plp_feats="ark:extract-segments \"scp,p:sed 's#PASSPHRASE#PASSPHRASE.$$#' $scp |\" $logdir/segments.JOB ark:- | compute-plp-feats $vtln_opts --verbose=2 --config=$plp_config ark:- ark:- |"
    pitch_feats="ark,s,cs:extract-segments \"scp,p:sed 's#PASSPHRASE#PASSPHRASE.$$#' $scp |\" $logdir/segments.JOB ark:- | compute-kaldi-pitch-feats --verbose=2 --config=$pitch_config ark:- ark:- | process-kaldi-pitch-feats $postprocess_config_opt ark:- ark:- |"
...
for n in $(seq $nj); do
  cat $plp_pitch_dir/raw_plp_pitch_$name.$n.scp
done > $data/feats.scp
