...
if $skip_done; then
  if [ -f $dir/.done.iter0 ]; then
    stage=$(ls $dir/.done.iter* | sed 's#\.done\.iter# #' | awk '{print $2}' | sort -nr | head -1)
    echo "found $dir/.done.iter$stage"
    echo "skip training until iter $stage"
  fi
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    for n in $(seq $this_num_jobs); do
      ...
      echo nnet-train$parallel_suffix $parallel_train_opts \
        ...
        $dir/$[$x+1].$n.mdl
    done > $dir/train.cmdlines

    $cmd $parallel_opts $dir/log/train.$x.log \
      -f $dir/train.cmdlines
