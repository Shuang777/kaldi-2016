...
case $feat_type in
  ...
  lda|fmllr) 
    ...
    [ -z $splice_opts ] && splice_opts=`cat $transform_dir/splice_opts 2>/dev/null` # frame-splicing options           
...
if [ ! -z $post_from ]; then
  ...
  $cmd JOB=1:$nj $dir/log/post.JOB.log \
    nnet-forward --feature-transform=$dnndir/final.feature_transform --apply-log=true \
