...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
    gmm-gselect --n=$num_gselect $srcdir/final.dubm "$feats" ark:- \| \
    fgmm-global-gselect-to-post --min-post=$min_post $srcdir/final.ubm "$feats" \
       ark,s,cs:- ark:- \| scale-post ark:- $posterior_scale ark:- \| \
    ivector3-extract --verbose=2 --select-parts=$select_parts --seg-parts=$seg_parts --bootstrap=$bootstrap \
      $srcdir/$ie "$feats" ark,s,cs:- ark:- \| \
    ivector-normalize-length ark:- ark,scp,t:$dir/ivector.JOB.ark,$dir/ivector.JOB.scp
fi
