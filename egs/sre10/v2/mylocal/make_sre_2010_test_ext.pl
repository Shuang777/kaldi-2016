...
if (@ARGV != 4) {
  print STDERR "Usage: $0 <path-to-SRE2010-eval> <data-dir> <trials> <task-name>\n";
  print STDERR "e.g. $0 /export/corpora5/SRE/SRE2010/eval/ data/sre10_test trials.ndx coreext-coreext\n";
  exit(1);
}

($db_base, $out_dir, $ndxfile, $task) = @ARGV;
...
open(WAVLIST, "<", "$ndxfile") or die "cannot open wav list";
