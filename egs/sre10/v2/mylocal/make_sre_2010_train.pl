...
if (@ARGV != 3) {
  print STDERR "Usage: $0 <path-to-SRE2010-eval> <data-dir> <task-name>\n";
  print STDERR "e.g. $0 /export/corpora5/SRE/SRE2010/eval/ data/sre10_train 10sec\n";
  exit(1);
}

($db_base, $out_dir, $task) = @ARGV;
...
open(WAVLIST, "<", "$db_base/train/$task.trn") or die "cannot open wav list";
