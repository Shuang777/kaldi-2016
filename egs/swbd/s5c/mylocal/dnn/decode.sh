...
case $feat_type in
  raw) feats="ark,s,cs:apply-cmvn-sliding --center=true scp:$sdata/JOB/feats.scp ark:- |"
  if [ -f $srcdir/delta_order ]; then
    delta_order=`cat $srcdir/delta_order 2>/dev/null`
    feats="$feats add-deltas --delta-order=$delta_order ark:- ark:- |"
  fi
    ;;
  lda) feats="ark,s,cs:apply-cmvn-sliding --center=true scp:$sdata/JOB/feats.scp ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $srcdir/final.mat ark:- ark:- |"
    ;;
