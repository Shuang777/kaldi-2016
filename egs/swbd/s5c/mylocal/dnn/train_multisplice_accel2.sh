...
if [ $stage -le -4 ]; then
  ...
  mylocal/dnn/get_lda.sh $lda_opts "${extra_opts[@]}" --left-context $first_left_context --right-context $first_right_context --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  mylocal/dnn/get_egs2.sh $egs_opts "${extra_opts[@]}" \
