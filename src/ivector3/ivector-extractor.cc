...
void IvectorExtractorUtteranceStats::AccStats(
    const MatrixBase<BaseFloat> &feats, 
    const Posterior &post, 
    const std::map<int32, int32> & selected_frames) {

  typedef std::vector<std::pair<int32, BaseFloat> > VecType;
  typedef std::vector<std::pair<int32, int32> > FramePair;
  ...
  for (std::map<int32, int32>::const_iterator it = selected_frames.begin(); it != selected_frames.end(); ++it) {
    ...
    if (S_.size() != 0) {
      outer_prod.SetZero();
      outer_prod.AddVec2(count, frame);
    }

    for (VecType::const_iterator iter = this_post.begin();
        iter != this_post.end(); ++iter) {
      int32 j = iter->first; // Gaussian index.
      KALDI_ASSERT(j >= 0 && j < num_gauss &&
                   "Out-of-range Gaussian (mismatched posteriors?)");
      double weight = iter->second;
      gamma_(j) += count * weight;
      X_.Row(j).AddVec(count * weight, frame);
      if (S_.size() != 0) {
        S_[j].AddSp(count * weight, outer_prod);
      }
    }
  } 
}
...
void IvectorExtractorStats::AccStatsForUtterance(const IvectorExtractor &extractor,
  ...
  utt_stats.AccStats(feats, post, std::vector<bool>());
...
void IvectorExtractor::ComputeConditionNumber(const Posterior &post) const {
  ...
  SubVector<double> q_vec(ivector_var.Data(), IvectorDim()*(IvectorDim()+1)/2);
  q_vec.AddMatVec(1.0, AT_Psi_inv_A_, kTrans, gamma, 0.0);   // A^T * Psi_inv * gamma * A

  Vector<double> s(ivector_dim);
  Matrix<double> p(ivector_dim, ivector_dim);
  ivector_var.Eig(&s, &p);
  KALDI_LOG << "Max_eig_value: " << s.Max() << " Min_eig_value: "  << s.Min() << " cond_num = " << s.Max()/s.Min();
}

void IvectorExtractor::ComputeConditionNumberAA() const {
  int32 num_gauss = NumGauss();
  int32 ivector_dim = IvectorDim();

  SpMatrix<double> AA(ivector_dim);
  for (int32 i = 0; i < num_gauss; i++) {
    AA.AddMat2(1.0, A_[i], kTrans, 1.0);
  }
